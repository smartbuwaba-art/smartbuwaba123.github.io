...
// Smart Buwaba V36 — safe startup: Firebase problems must never freeze navigation.
...
try{
 if(window.firebase && typeof firebase.initializeApp==="function"){
   ...
   firebaseReady=true;
 }else console.warn("Smart Buwaba: Firebase SDK unavailable; offline navigation enabled.");
}catch(e){
 console.error("Smart Buwaba Firebase startup failed:",e);
...
async function loadPosts(){
 if(!firebaseReady || !db){ posts=[...defaultPosts]; renderHome(); renderVideos(); return; }
 try{
 ...
 return {
  ...
  viewCount:data.viewCount ?? 0
   ...
   console.error("Could not load videos from Firestore:",e);
   posts=[...defaultPosts];
 }
 renderHome();renderVideos();
}
...
if(firebaseReady && firebase.auth){ firebase.auth().onAuthStateChanged(u=>{currentUser=u;}); }
...
if(firebaseReady) loadPosts();
else { posts=[...defaultPosts]; renderHome(); renderVideos(); }

/* ===== SMART BUWABA V2 ===== */
let storage = null; if(firebaseReady && firebase.storage) storage = firebase.storage();
...
if(firebaseReady && firebase.auth) firebase.auth().onAuthStateChanged(async u=>{
 currentUser=u;
 if($("userLabel")){
...
function handleAIImageSelected(file){
 if(!file)return;
 const prompt=$("aiImagePrompt");
 if(prompt && !prompt.value.trim()) prompt.value="Edit this photo in a creative, professional Smart Buwaba style.";
 const st=$("aiImageStatus");
 if(st)st.textContent="Photo selected ✓ Write your edit request, then tap Generate Image.";
 const result=$("aiImageResult");
 if(result){ const url=URL.createObjectURL(file); result.innerHTML='<img src="'+url+'" alt="Selected photo" style="max-width:100%;max-height:260px;border-radius:14px;display:block;margin-bottom:9px">'; }
 $("aiImagePrompt")?.scrollIntoView({behavior:"smooth",block:"center"});
}
...
async function askAI(){
 ...
 const btn=$("aiAskBtn");if(btn){btn.disabled=true;btn.textContent="…";}
 const status=$("aiStatus");if(status)status.textContent="AI: thinking…";
 try{
  if(wantsImageGeneration(q)){
   await generateAIImageFromPrompt(imagePromptFromChat(q));
   addAI("Ndalenga chithunzi chako. Chawoneka pansipa pa Image AI.","ai");
   if(status)status.textContent="AI: image created ✓";
   return;
  }
  ...
  if(status)status.textContent="AI: connected ✓";
 }catch(e){
  console.error("AI error",e);
  if(status)status.textContent="AI: connection failed";
  if(!wantsImageGeneration(q))addAI("Pepani bro, AI server sikuyankha pano. Tionenso backend/Vercel connection.","ai");
...
function sbImageEndpoint(){
 ...
 return /^https?:\/\//i.test(v)?v:SB_DEFAULT_IMAGE_BACKEND;
}
...
function wantsImageGeneration(text){
 ...
 return (/\b(create|generate|make|draw|design|render|visualize|paint|edit|panga|pangani|jambula)\b/.test(q) && /\b(image|picture|photo|portrait|drawing|art|illustration|chithunzi|chithuz|zojambula)\b/.test(q));
}
function imagePromptFromChat(text){return String(text||"").replace(/^\s*(please\s+)?(create|generate|make|draw|design|render|visualize|paint|edit|panga|pangani|jambula)\s+(an?\s+)?(image|picture|photo|portrait|drawing|art|illustration|chithunzi|chithuz|zojambula)\s*(of|showing|that|:)?\s*/i,"").trim()||String(text||"").trim();}
async function generateAIImageFromPrompt(prompt,file=null){
 const st=$("aiImageStatus"),out=$("aiImageResult");
 if(st)st.textContent="Image AI: creating…";
 try{
  const body={prompt:String(prompt||"").slice(0,4000)};
  if(file)body.imageData=await fileToDataURL(file);
  ...
  out.innerHTML='<img src="'+data.image+'" alt="Smart Buwaba AI image" style="width:100%;max-width:760px;border-radius:16px;display:block"><div class="actions" style="margin-top:9px"><a class="btn" href="'+data.image+'" download="smart-buwaba-ai-image.png">⬇ Save Image</a></div>';
  if(st)st.textContent="Image AI: ready ✓";
  return data.image;
 }catch(e){
  console.error(e);
  if(st)st.textContent="Image AI: connection failed";
  out.innerHTML='<div class="note">Image AI failed: '+safeText(e.message||e)+'<br><br>Backend ikufunika kukhala deployed ndi OPENAI_API_KEY.</div>';
  throw e;
 }
}
...
document.addEventListener("DOMContentLoaded",()=>{
  const boot=$("sbBootNotice"); if(boot)boot.style.display="none";
...
setTimeout(()=>{ const boot=document.getElementById("sbBootNotice"); if(boot)boot.style.display="none"; },3000);
